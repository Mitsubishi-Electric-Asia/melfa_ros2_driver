...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            default_value="melfa_rv80fr_moveit_config",
    ...
    moveit_config = (
        MoveItConfigsBuilder("rv80fr", package_name="melfa_rv80fr_moveit_config")
        .robot_description(
            file_path="config/rv80fr.urdf.xacro",
        )
    ...
    rviz_config_file = PathJoinSubstitution(
        [FindPackageShare(moveit_config_package), "rviz", "rv80fr_moveit.rviz"]
    )
    rviz_node = Node(
        package="rviz2",
    ...
    servo_params = (
        ParameterBuilder("melfa_rv80fr_moveit_config")
        .yaml(
            parameter_namespace="moveit_servo",
            file_path="config/rv80fr_servo.yaml",
        )
        .to_dict()
    )
